...
//
// Table class for app_time
//
class crapp_time extends crTableBase {
	...
	function __construct() {
		...
		$this->TableVar = 'app_time';
		$this->TableName = 'app_time';
		$this->TableType = 'VIEW';
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// app_id
		$this->app_id = new crField('app_time', 'app_time', 'x_app_id', 'app_id', '`app_id`', 3, EWR_DATATYPE_NUMBER, -1);
		...
		// username
		$this->username = new crField('app_time', 'app_time', 'x_username', 'username', '`username`', 200, EWR_DATATYPE_STRING, -1);
		...
		// doc_username
		$this->doc_username = new crField('app_time', 'app_time', 'x_doc_username', 'doc_username', '`doc_username`', 200, EWR_DATATYPE_STRING, -1);
		...
		// specialization
		$this->specialization = new crField('app_time', 'app_time', 'x_specialization', 'specialization', '`specialization`', 200, EWR_DATATYPE_STRING, -1);
		...
		// fees
		$this->fees = new crField('app_time', 'app_time', 'x_fees', 'fees', '`fees`', 3, EWR_DATATYPE_NUMBER, -1);
		...
		// date
		$this->date = new crField('app_time', 'app_time', 'x_date', 'date', '`date`', 133, EWR_DATATYPE_DATE, 0);
		...
		// time
		$this->time = new crField('app_time', 'app_time', 'x_time', 'time', '`time`', 200, EWR_DATATYPE_STRING, -1);
		$this->time->Sortable = TRUE; // Allow sort
		$this->fields['time'] = &$this->time;
		$this->time->DateFilter = "";
		$this->time->SqlSelect = "SELECT DISTINCT `time`, `time` AS `DispFld` FROM " . $this->getSqlFrom();
		$this->time->SqlOrderBy = "`time`";

		// reason
		$this->reason = new crField('app_time', 'app_time', 'x_reason', 'reason', '`reason`', 200, EWR_DATATYPE_STRING, -1);
		...
		// postingDate
		$this->postingDate = new crField('app_time', 'app_time', 'x_postingDate', 'postingDate', '`postingDate`', 135, EWR_DATATYPE_DATE, 0);
	...
	function getSqlFrom() {
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`app_time`";
	}
	...
	function getSqlSelectAgg() {
		return ($this->_SqlSelectAgg <> "") ? $this->_SqlSelectAgg : "SELECT COUNT(*) AS `cnt_time` FROM " . $this->getSqlFrom();
	}
